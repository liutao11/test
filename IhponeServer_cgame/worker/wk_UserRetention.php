        ...
        if(@$cookie['serverIndex'] && $cookie['serverIndex']!=0 ){
            ...
            for($i=0;$i<$dayLeng;$i++) {
                ...
                if ($jstime < strtotime(date("Y-m-d 23:59:59"))) {
                    $StartDayTableName_CDay = date("Y-m-d", $StartDay_s + 24 * 3600);
                    $sql = "select aa.sAccount from {$DBGameName}.dbo.mir_hum_info as aa where dUpdateTime >= '{$StartDayTableName_CDay}' GROUP BY aa.sAccount";
                    $CDayUserResult = $dblinkS->query($sql, 24 * 3600);

                    foreach ($CDayUserResult as $key => $item) {
                        foreach ($item as $kk => $vv) {
                            if (@in_array($vv['sAccount'], $thisUserDataArray[$StartDayTableName])) {
                ...
                if ($jstime < strtotime(date("Y-m-d 23:59:59"))) {
                    $StartDayTableName_CDay = date("Y-m-d", $StartDay_s + 24 * 3600 * 2);
                    $sql = "select aa.sAccount from {$DBGameName}.dbo.mir_hum_info as aa where dUpdateTime >= '{$StartDayTableName_CDay}' GROUP BY aa.sAccount";
                    $CDayUserResult = $dblinkS->query($sql, 24 * 3600);
                ...
                if ($jstime < strtotime(date("Y-m-d 23:59:59"))) {
                    $StartDayTableName_CDay = date("Y-m-d", $StartDay_s + 24 * 3600 * 3);
                    $sql = "select aa.sAccount from {$DBGameName}.dbo.mir_hum_info as aa where dUpdateTime >= '{$StartDayTableName_CDay}' GROUP BY aa.sAccount";
                    $CDayUserResult = $dblinkS->query($sql, 24 * 3600);
                ...
                if ($jstime < strtotime(date("Y-m-d 23:59:59"))) {
                    ...
                    $StartDayTableName_CDay = date("Y-m-d", $StartDay_s + 24 * 3600 * 4);
                    $sql = "select aa.sAccount from {$DBGameName}.dbo.mir_hum_info as aa where dUpdateTime >= '{$StartDayTableName_CDay}' GROUP BY aa.sAccount";
                    $CDayUserResult = $dblinkS->query($sql, 24 * 3600);
                ...
                if ($jstime < strtotime(date("Y-m-d 23:59:59"))) {
                    ...
                    $StartDayTableName_CDay= date("Y-m-d", $StartDay_s + 24 * 3600 * 5);
                    $sql = "select aa.sAccount from {$DBGameName}.dbo.mir_hum_info as aa where dUpdateTime >= '{$StartDayTableName_CDay}' GROUP BY aa.sAccount";
                    $CDayUserResult = $dblinkS->query($sql, 24 * 3600);
                ...
                if ($jstime < strtotime(date("Y-m-d 23:59:59"))) {
                    ...
                    $StartDayTableName_CDay = date("Y-m-d", $StartDay_s + 24 * 3600 * 6);
                    $sql = "select aa.sAccount from {$DBGameName}.dbo.mir_hum_info as aa where dUpdateTime >= '{$StartDayTableName_CDay}' GROUP BY aa.sAccount";
                    $CDayUserResult = $dblinkS->query($sql, 24 * 3600);
                ...
                if ($jstime < strtotime(date("Y-m-d 23:59:59"))) {
                    ...
                    $StartDayTableName_CDay = date("Y-m-d", $jstime);
                    $sql = "select aa.sAccount from {$DBGameName}.dbo.mir_hum_info as aa where dUpdateTime >= '{$StartDayTableName_CDay}' GROUP BY aa.sAccount";
                    $CDayUserResult = $dblinkS->query($sql, 24 * 3600);
