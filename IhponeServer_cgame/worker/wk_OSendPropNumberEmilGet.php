...
class wk_OSendPropNumberEmilGet extends  e_OperatorsWorker{
        ...
        foreach($arg['serverS'] as $kk=>$vv){
            ...
            foreach($role_array as $role_only) {
                ...
                foreach($arg['PropS'] as $vv){
                    @$PropStyleValues=0;
                    @$PropClockValues=isset($vv['PropClockValues'])?$vv['PropClockValues']:1;
                    $PropNum=$vv['PropNum']?$vv['PropNum']:1;
                    $sItem .= $vv['PropName'].'*'.$PropStyleValues.'*'.$PropNum.'*'.$PropClockValues.',';
                }
